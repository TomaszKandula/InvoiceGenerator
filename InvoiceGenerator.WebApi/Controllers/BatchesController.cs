...
public class BatchesController : BaseController
{
    ...
    public async Task<OrderInvoiceBatchCommandResult> OrderInvoiceBatch([FromBody] OrderInvoiceBatchDto payload, [FromHeader(Name = HeaderName)] string privateKey) 
    ...
    public async Task<GetBatchProcessingStatusQueryResult> GetBatchProcessingStatus([FromQuery] Guid processBatchKey, [FromHeader(Name = HeaderName)] string privateKey) =>
        await Mediator.Send(new GetBatchProcessingStatusQuery { ProcessBatchKey = processBatchKey });

    [HttpGet]
    [ProducesResponseType(typeof(FileContentResult), StatusCodes.Status200OK)]
    public async Task<FileContentResult> GetIssuedBatchInvoice([FromQuery] string invoiceNumber, [FromHeader(Name = HeaderName)] string privateKey) =>
    ...
    public async Task<IEnumerable<GetBatchProcessingStatusListQueryResult>> GetBatchProcessingStatusList([FromHeader(Name = HeaderName)] string privateKey) =>
        await Mediator.Send(new GetBatchProcessingStatusListQuery { FilterBy = string.Empty });

    [HttpGet("{status}")]
    [ProducesResponseType(typeof(IEnumerable<GetBatchProcessingStatusListQueryResult>), StatusCodes.Status200OK)]
    public async Task<IEnumerable<GetBatchProcessingStatusListQueryResult>> GetBatchProcessingStatusCode([FromRoute] string status, [FromHeader(Name = HeaderName)] string privateKey) =>
